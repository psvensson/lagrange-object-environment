...
| Renderer-specific graphics and host resources | `RendererAdapter` | planned | Owns concrete browser/native GPU, device, queue, surface and frame resources plus Component/WIT graphics host providers; does not own semantic Presentation or composition policy. |
...
| Object Environment -> Lagrange Images | `ImageClientAdapter` (`src/image-client-adapter.js`) | current for the leaf/edge object loop; Perspective persistence blocked (see below) | Sole environment-side adapter for public image APIs, observation, authorized operations and durable Perspective projection. Lagrange Images still owns its own public semantics. Composes the cores: observation `src/image-observation.js` (ADR 0009), command dispatch `src/command-dispatcher.js` (ADR 0010), Perspective projection `src/perspective-projection.js` (ADR 0012). The ADR 0012 child-object Perspective form is expressible through the current creation lane; adapter save/load wiring is planned, and its indexed *ordering* awaits `lagrange-images#119`. The leaf/edge object loop is proven against a real runtime. |
| Perspective -> durable image representation | `ImageClientAdapter` (`src/perspective-projection.js`) | current: projection core (ADR 0012); save/load wiring and indexed ordering planned | Owns the Perspective <-> ordinary-image-object encode/decode contract of ADR 0012 (superseding ADR 0008). `Perspective` owns semantics; Lagrange Images owns storage. The pure projection is implemented; the ADR 0012 child-object form is expressible through the current creation lane, but the adapter does not yet implement save/load, and the indexed *ordering* awaits `lagrange-images#119`. |
| Presentation -> renderer | `RendererAdapter` | planned | Converts semantic presentation output into renderer-specific view operations without moving semantics into the renderer; for Component-backed presentations, supplies the exact-version graphics/surface host interfaces the renderer declares. |
| Compositor -> concrete render surface | `RendererAdapter` | planned | Owns translation from logical surface/view lifecycle requests into concrete browser/native surfaces and GPU resources; `Compositor` owns logical arrangement and lifetime intent, while `RendererAdapter` owns host-resource realization. |
...
| Session -> composition/render lifecycle | `Compositor` | planned | Owns ephemeral logical composition, focus, layout, visibility and view lifetime; requests concrete surfaces from `RendererAdapter` and does not implement GPU/WIT semantics. Durable intent is promoted explicitly to Perspective. |
