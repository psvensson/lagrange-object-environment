...
- **Failed-open compensation is Compositor-owned.** `openView` sequences `createSurface` then `attachPresentation`. If creation returns a validated handle but attach fails, the Compositor awaits exactly one best-effort `destroySurface` attempt before it publishes the handle-free lost logical view, and it preserves the attach failure as the primary error. A RendererAdapter realizes each individual lifecycle operation and maps its host-resource failures; it does not duplicate or independently decide this multi-call rollback policy.
