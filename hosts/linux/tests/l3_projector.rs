...
fn rust_projector_matches_the_checked_in_fixtures() {
    let cases: Vec<(&str, Value)> = vec![
        ...
        // The SAME method as the browser DISPLAYS it (E3, Bead eij.3): identical
        // Images description plus the ONE transient input NativeSmalltalkBrowser
        // threads onto a native-method descriptor. A second fixture beside the
        // read-only one, exactly as project-editable sits beside project -- and
        // the cross-host proof that the GTK lane renders E3's affordance from the
        // same descriptor the browser lane does.
        ("native-method-editable.json", json!({"kind":"native-method","subject":{"kind":"native-method","imageId":"img","classRef":ref_("smalltalk/class/BrowseChild"),"selector":"childFirst"},"parameters":{"smalltalkMethod":{"format":"smalltalk-method-description/v1","class":ref_("smalltalk/class/BrowseChild"),"side":"instance","selector":"childFirst","method":ref_("smalltalk/class/BrowseChild/method/Y2hpbGRGaXJzdA"),"source":null,"provenance":null},"inputs":[{"role":"native-method-source","label":"New source","submitLabel":"Replace"}]}})),
